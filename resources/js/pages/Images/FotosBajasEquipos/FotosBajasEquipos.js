...
function FotosBajasEquipos(props) {
    ...
    const sendHandler = () => {
        ...
        fetch('https://simagenes.gimcloud.com/api/fotosbajashistoricos/post', {
            method: 'POST',
            body: formdata
        })
            .then(res => res.text())
            .then(res => swal("Foto del Equipo", "Ingreso Dcto Baja Historico del Equipo OK", "success", { button: "Aceptar" }))
            .catch(err => {
                swal("Foto del Equipo", "Error Grabando Dcto Baja Historico", "error", { button: "Aceptar" })
            })
    ...
    const ordenGrabar = (
        <div className={styles.modal}>
            <Typography align="center" className={styles.typography} variant="button" display="block">
                Agregar Dcto Baja Historico del Equipo
            </Typography>
            <br />
            <Grid container spacing={2} >
                <Grid item xs={12} md={12}>
                    <TextField name="" label="Nombre del Dcto" fullWidth onChange={(e) => setNombreFoto(e.target.value)} />
                </Grid>
                ...
                <Grid item xs={12} md={6}>
                    <Button className={styles.button} variant="contained" startIcon={<SaveIcon />} onClick={sendHandler} >Agregar Dcto Baja</Button>
                </Grid>
            </Grid>
        </div>
    )
...
export default FotosBajasEquipos;