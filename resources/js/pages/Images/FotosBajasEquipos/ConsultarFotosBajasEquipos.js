...
import fotosbajasequiposServices from "../../../services/Mantenimiento/FotosBajasHistoricos";
...
function ConsultarFotosBajasEquipos(props) {
  ...
  useEffect(() => {
    async function fetchDataFotoEquipo() {
      const res = await fotosbajasequiposServices.listfotosequipos(codigoequipo);
  ...
  const borrarFotoEquipo = async () => {
    ...
    const res = await fotosbajasequiposServices.delete(fotoEquipoSeleccionado.id);

    if (res.success) {
      swal({
        title: "Consultar Fotos Equipos",
        text: "Dcto Baja del Equipo Borrada de forma Correcta!",
        icon: "success",
        button: "Aceptar"
      });
      ...
      swal({
        title: "Consultar Fotos Equipos",
        text: "Error Borrando Dcto Baja Historico del Equipo!",
        icon: "error",
        button: "Aceptar"
      });
      console.log(res.message);
  ...
  const mostrarFoto = (nombrefoto) => {
    window.open("https://imagenes.gimcloud.co/" + nombrefoto, "Foto Maquina")
  }
    ...
    <div className="App">
      <div className={styles.modal}>
        ...
        <MaterialTable
          ...
          title="DCTOS BAJA EQUIPOS"
          actions={[
            {
              icon: FileCopyIcon,
              tooltip: 'Consultar Dcto',
...
export default ConsultarFotosBajasEquipos;