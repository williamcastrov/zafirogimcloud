...
/*
    useEffect(() => {
        async function fetchDataDashboard() {
            const res = await contratosServices.listconsoestcontratos();
...
*/
    useEffect(() => {
        ...
        async function fetchDataTotalContratos() {
            const res = await contratosServices.sumatotalcontratos();
            console.log("TOTAL CONTRATOS : ", res.data);

            async function fetchDataDashboard() {
                const rest = await contratosServices.listconsoestcontratos(res.data[0].totalcontratos);
                console.log("DATOS ESTADOS CONTRATOS  : ", rest.data);
                setListContratos(rest.data);
                //console.log(listDashboard);
            }
            fetchDataDashboard();
        }
        fetchDataTotalContratos();
    ...
    return (
        <div style={{ height: 400, width: '100%' }}>
            <MaterialTable
                columns={columnas}
                data={listContratos}
                title="ESTADOS CONTRATOS DE LOS EQUIPOS"
                options={{
                    actionsColumnIndex: -1
                }}
                localization={{
...
export default EstadosContratos;