...
function CrearOrdenes(props) {
  ...
  const [listarContactosCliente, setListarContactosCliente] = useState([]);
  ...
  const [contacto, setContacto] = useState(24);
  ...
  const [ordenSeleccionado, setOrdenSeleccionado] = useState({
    ...
    'tiempoorden_otr': 0,
    'iniciaparomaquina_otr': fechatransporte,
    'finalparomaquina_otr': fechatransporte,
    'tiempoparo_otr': 0
  })
  ...
  const DatosEquipos = (equipo) => {
    async function fetchLeerDatoEquipo() {
      ...
      async function fetchDataContactos() {
        ...
        //console.log("CONTACTOS : ", rest.data)
        if (!res.success) {
          //swal("Contactos", "Cliente Seleccionado no tiene Contactos!", "warning", { button: "Aceptar" });
        }
      }
...

      {
        ...
        cliente ? (
          setOrdenSeleccionado([{
            ...
            operariodos_otr: asignaOperario,
            contactocliente_otr: codigocontacto,
            ...
            tiempotransporte_otr: 0,
            tiempoorden_otr: 0,
            iniciaparomaquina_otr: ordenSeleccionado.iniciaparomaquina_otr,
            finalparomaquina_otr: fechatransporte,
            tiempoparo_otr: 0
          }])
        )
  ...
  const ordenInsertar = (
    <div className={styles.modal}>
      ...
      <Grid container spacing={2} >
        ...
        <Grid item xs={12} md={3}>
          <TextField type="date" InputLabelProps={{ shrink: true }} name="iniciaparomaquina_otr"
            defaultValue={Moment(fechatransporte).format('YYYY-MM-DD')} label="Fecha Paro Maquina"
            fullWidth onChange={handleChange} />
        </Grid>
            ...
            >
              <MenuItem value=""> <em>None</em> </MenuItem>
              {
                listarContactosCliente ?
                  listarContactosCliente.map((itemselect) => {
                    return (
    ...
    <div className="App" >
      <div className={styles.modal}>
        ...
        <Grid container spacing={2} >
          ...
          <Grid item xs={12} md={3}>
            <TextField type="date" InputLabelProps={{ shrink: true }} name="iniciaparomaquina_otr"
              defaultValue={Moment(ordenSeleccionado.iniciaparomaquina_otr).format('YYYY-MM-DD')} label="Fecha Paro Maquina"
              fullWidth onChange={handleChange} />
          </Grid>
          <Grid item xs={12} md={4}> <TextField type="date" InputLabelProps={{ shrink: true }} name="fechaprogramada_otr"
