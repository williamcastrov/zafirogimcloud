...
function RevisarAprobarActividades(props) {
  const { id_otr, nombre_emp, razonsocial_cli, telefono_cli, nombre_ciu, email_cli, descripcion_mar, modelo_dequ,
    ...
    tiempoorden_otr, estado_otr, id_actividad
  ...
  useEffect(() => {
    async function fetchDataUnCumplimiento() {
      const res = await cumplimientooservServices.listUnCumplimiento(id_actividad);
  ...
  const grabarCumplimiento = async () => {
    // Cumplimiento o Registro de actividades se realiza desde Remote Gim Cloud
  }
  ...
  const CerrarOrden = (
    <div className={styles.modal}>
      <p>Estás seguro que deseas Cerrar la Actividad de la OT <b> {id_actividad} </b>? </p>
      <div align="right">
  ...
  return (
    <div className="App">
      <br />
      <Typography align="center" className={styles.typography} variant="button" display="block" >
        ORDEN DE SERVICIO # {props.ordenSeleccionado.id_otr} {"-"} ACTIVIDAD # {props.ordenSeleccionado.id_actividad}
      </Typography>
      ...
      <div>
        {
          activo ? (
            <div>
              ...
              <Button className={styles.button3} onClick={() => abrirCerrarModalCerrarOrden()} >Cerrar Actividad </Button>
              <Button className={styles.button2} onClick={() => pasarARevision()} >Actualizar Tiempos </Button>
            </div>
          )
            :
            (
...
export default RevisarAprobarActividades;
