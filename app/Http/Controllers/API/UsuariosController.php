...
class UsuariosController extends Controller
{
    ...
    public function leer_usuario($uidfirebase_usu){
      try {
        $data = DB::select("SELECT t0.*, t1.nombre_est, t2.nombre_ciu, t3.descripcion_und
        FROM usuarios as t0 INNER JOIN estados as t1 INNER JOIN ciudades as t2  INNER JOIN unidades as t3
        WHERE t0.estado_usu = t1.id_est and t0.ciudad_usu = t2.id_ciu and t0.tipo_usu IN (10,11,12,13,17,18,19,20,21,22,23) and t0.tipo_usu = t3.id_und and
    ...
    public function habilitaralertas($id_usu){
      try {
        $res = DB::update('update usuarios set dashboard_usu = 1 where id_usu = ?', [$id_usu]);
    ...
    public function habilitarindicadores($id_usu){
      try {
        $res = DB::update('update usuarios set dashboard_usu = 2 where id_usu = ?', [$id_usu]);
